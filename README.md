...
| 6 | 孔位冲突检测 | 同一孔位重复分配拦截 |
| 7 | 创建任务 | 多任务创建 |
| 8 | 生成配液方案 | 正常体积和小体积方案生成 |
| 9 | 正常体积批准 | 无警告时直接批准 |
| 10 | 最小移液拦截 | 低于最小移液体积时拦截，不扣库存 |
| 11 | 偏差备注后批准 | 添加偏差备注后可强制批准 |
| 12 | 库存扣减验证 | 批准后试剂、引物库存扣减 |
| 13 | 撤销确认 | 撤销后库存退回 |
| 14 | 驳回任务 | 驳回不扣减库存 |
| 15 | 历史记录与导出 | 历史记录完整性和 JSON/CSV 导出 |
| 16 | 报告导出 | 配液报告导出 |
| 17 | 库存不足拦截 | 库存不足时拦截，不预占库存 |
| 18 | 重启后数据一致性 | 模拟重启验证数据持久化 |
...
| GET | `/api/reports/<task_id>/json` | 导出任务报告 JSON |
| GET | `/api/reports/<task_id>/csv` | 导出任务报告 CSV |
